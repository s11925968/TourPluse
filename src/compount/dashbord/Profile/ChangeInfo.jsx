...
export default function Creaetoperator() {
  ...
  const formik = useFormik({
    initialValues: {
      name: "", // Use the name from dataProfile if available, otherwise use an empty string
      address:"",
      phoneNumber:"",
      image: "",
      description: "",
    ...
    onSubmit: async (users) => {
      ...
      try {
        ...
        const { data } = await axios.patch(
          `${import.meta.env.VITE_URL_LINK}/operator/changeProfile/${_id}`,
          formdata,
          {
            headers: {
        ...
        if (data.message === "success") {
          formik.resetForm();
          toast.success("You successfully updated the operator", {
            position: "top-center",
            autoClose: 5000,
        ...
        setErrorBackend(error.response?.data?.message || "An error occurred");
      } finally {
        setLoading(false);
      }
    },
